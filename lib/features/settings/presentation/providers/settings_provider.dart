...
// ============================================================================
// Bluetooth Lyrics Provider (客户端本地偏好)
// ============================================================================
...
class BluetoothLyricsEnabledNotifier extends Notifier<bool> {
  ...
  Future<void> setValue(bool value) async {
    ...
    try {
      final prefs = await ref.read(appPreferencesProvider.future);
      await prefs.setBluetoothLyricsEnabled(value);
    } catch (_) {}
  }
}
...
/// 蓝牙歌词兼容模式（障眼法）Notifer
class BluetoothLyricsCompatModeNotifier extends Notifier<bool> {
  @override
  bool build() {
    _load();
    return false;
  ...
  Future<void> _load() async {
    try {
      final prefs = await ref.read(appPreferencesProvider.future);
      state = prefs.getBluetoothLyricsCompatMode();
    } catch (_) {
      state = false;
    }
  }
  ...
  Future<void> setValue(bool value) async {
    ...
    try {
      final prefs = await ref.read(appPreferencesProvider.future);
      await prefs.setBluetoothLyricsCompatMode(value);
    } catch (_) {}
  }
}

/// 蓝牙歌词兼容模式 Provider
final bluetoothLyricsCompatModeProvider =
    NotifierProvider<BluetoothLyricsCompatModeNotifier, bool>(
      BluetoothLyricsCompatModeNotifier.new,
    );
